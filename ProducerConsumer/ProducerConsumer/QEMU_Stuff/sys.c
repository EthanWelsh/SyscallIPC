...
asmlinkage long sys_setpriority(int which, int who, int niceval)
{
    ...
    switch (which)
    {
        ...
        case PRIO_USER:
            ...
            if (!who)
                who = current->uid;
            else if ((who != current->uid) && !(user = find_user(who)))
 ...
 */
asmlinkage long sys_reboot(int magic1, int magic2, unsigned int cmd, void __user
...
if (
strncpy_from_user(&buffer[0], arg,
sizeof(buffer) - 1) < 0) {
unlock_kernel();
...
asmlinkage long sys_getresuid(uid_t __user
...
if (!(
retval = put_user(current->uid, ruid)
) &&
!(
...
asmlinkage long sys_getresgid(gid_t __user
...
asmlinkage long sys_times(struct tms __user
...
tmp.
tms_utime = cputime_to_clock_t(utime);
tmp.
tms_stime = cputime_to_clock_t(stime);
tmp.
tms_cutime = cputime_to_clock_t(cutime);
tmp.
tms_cstime = cputime_to_clock_t(cstime);
if (
copy_to_user(tbuf, &tmp,
sizeof(struct tms)))
return -
EFAULT;
}
...
if (
copy_to_user(grouplist
+off, group_info->blocks[i], len))
return -
EFAULT;
...
if (
copy_from_user(group_info
->blocks[i], grouplist+off, len))
return -
EFAULT;
...
static void groups_sort(struct group_info *group_info)
{
    ...
    for (stride = 1; stride < gidsetsize; stride = 3 * stride + 1); /* nothing */
    stride /= 3;
...
asmlinkage long sys_getgroups(int gidsetsize, gid_t __user
...
if (
groups_to_user(grouplist, current
->group_info)) {
i = -EFAULT;
goto
out;
...
asmlinkage long sys_sethostname(char __user
 ...
 * uname()
 */
asmlinkage long sys_setdomainname(char __user
        ...
        unsigned long arg4, unsigned long arg5)
{
    ...
    switch (option)
    {
        ...
        case PR_GET_PDEATHSIG:
            error = put_user(current->pdeath_signal,(
            int __user
            *)arg2);
            break;
